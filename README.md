...
第一次跑会自动在 `/usr/local/bin/init` 创建软链接。
...
| 用途 | 路径 |
|------|------|
| 脚本本身 | 任意位置（启动时拷贝到 `/usr/local/bin/init`） |
| 快捷指令软链接 | `/usr/local/bin/init` |
...
**Q: 怎么卸载脚本？**
A: 删文件 + `rm /usr/local/bin/init` 即可，**不会**还原已装的服务和配置。
