...
- **11 个核心功能**：基础环境、Docker、SSH 加固、防火墙、服务管理、实用脚本
...
第三方作者脚本快捷入口（本站不维护，仅提供一键运行）：

- **1. 硬件检测**（xykt/HardwareQuality）：CPU/内存/磁盘/网络综合跑分
- **2. IP 质量**（xykt/IPQuality）：IP 黑名单/欺诈分/出口类型检测
- **3. 流媒体解锁**（xykt/RegionRestrictionCheck）：Netflix/Disney+/YouTube 等解锁检测
- **4. 自定义脚本**：手动粘贴任意脚本直链运行
